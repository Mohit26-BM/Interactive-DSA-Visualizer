...
export default function DirectedGraphPage() {
  const [graphState, setGraphState] = useState<GraphState>(() => buildDefaultGraph("directed"));
    ...
    <div className="min-h-screen bg-gray-950 text-white">
      <header className="border-b border-gray-800 px-6 py-4 flex items-center gap-4">
        ...
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 bg-purple-700 rounded-lg flex items-center justify-center text-sm font-bold">○→○</div>
          <h1 className="text-xl font-bold">Directed Graph</h1>
          <span className="text-xs bg-purple-500/10 border border-purple-500/20 text-purple-300 px-2 py-0.5 rounded-full">
            one-way edges · DAG · reachability
          </span>
        </div>
        {steps.length > 0 && step === steps.length - 1 && (
          <button type="button" onClick={handleApplyResult} className="ml-auto px-4 py-1.5 bg-purple-600 hover:bg-purple-500 text-white rounded-lg text-sm font-medium transition-colors">
            Apply Result
          </button>
        )}
      </header>
      ...
      <div className="max-w-6xl mx-auto p-6 space-y-4">
        ...
        <div className="bg-gray-900 border border-gray-700 rounded-xl overflow-hidden">
          <div className="px-4 py-3 border-b border-gray-800 flex items-center justify-between">
            ...
            <span className="text-xs text-gray-500">
              {displayState.vertices.length} vertices · {displayState.edges.length} edges · directed
            </span>
          </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
          <GraphControls directed={true} onRun={handleRun} />
          <div className="bg-gray-900 border border-gray-700 rounded-xl p-4 space-y-3">
            <h3 className="text-sm font-semibold text-gray-300">Complexity Analysis</h3>
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-gray-800 rounded-lg p-3">
                <div className="text-xs text-gray-500 mb-1">Time</div>
                <div className="text-lg font-bold text-purple-400">{complexity?.time}</div>
              </div>
              <div className="bg-gray-800 rounded-lg p-3">
